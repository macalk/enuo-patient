...
#import "ZXDDownMenu.h"
#import "Macros.h"
#import <UIKit/UIKit.h>
#import <UIImageView+WebCache.h>

@interface ZXDDownMenu ()
...
- (void)remover{
    ...
    [UIView animateWithDuration:self.hideAnmationTime animations:^{
...
- (void)createMenuViewWithData:(NSArray *)data{
    ...
    self.hideAnmationTime = self.hideAnmationTime ? self.hideAnmationTime : hideAnimationDefalutTime;
...
- (void)showFirstTableView:(UIButton *)btn{
    ...
    if (self.lastSelectedIndex != btn.tag && self.lastSelectedIndex !=-1) {
        ...
        [UIView animateWithDuration:self.hideAnmationTime/2 animations:^{
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == self.tableFirst) {
        NSLog(@"self.dataSourceFirst.count = %ld",self.dataSourceFirst.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.tableFirst) {
        ...
        if ([self.dataImageFirst[0]isEqualToString:@"点诺"]) {
            cell1.imageView.image = [UIImage imageNamed:self.dataImageFirst[indexPath.row]];
        }else{
            NSString *strr = [NSString stringWithFormat:urlPicture,self.dataImageFirst[indexPath.row]];
            [cell1.imageView sd_setImageWithURL:[NSURL URLWithString:strr] placeholderImage:nil];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    void (^complete)(void) = ^(void){
        ...
        [UIView animateWithDuration:weakSelf.hideAnmationTime animations:^{
    ...
    if (tableView == self.tableFirst) {
        ...
        if (self.isNet&&![self.dataSourceFirst[0]isEqualToString:@"诺"]) {
