...
@property (nonatomic,strong) NSMutableArray *dataImageFirst;

@property (nonatomic,strong) NSMutableArray *data;
@property (nonatomic,strong) NSMutableArray *allData;
@property (nonatomic,strong) NSMutableArray *allDataSource;
@property (nonatomic,strong) NSMutableArray *allImageSource;
...
- (void)changeMenuDataWithIndex:(NSInteger)index{
    ...
    [self createWithFirstData:self.allDataSource[index][0]firstImage:self.allImageSource[index]];
...
- (void)createWithFirstData:(NSArray *)dataFirst firstImage:(NSArray *)imageFirst{
    self.dataSourceFirst = [NSMutableArray arrayWithArray:dataFirst];
    self.dataImageFirst = [NSMutableArray arrayWithArray:imageFirst];
    [self.tableFirst reloadData];
}

//- (void)creatWithFirstImageData:(NSArray *)imageFirst{
//    self.dataImageFirst = [NSMutableArray arrayWithArray:imageFirst];
//    [self.tableFirst reloadData]
//    
//}
...
- (void)createTableViewFirst{
    ...
    self.tableFirst = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.backView.frame),self.frame.size.width, 0)];
     self.tableFirst.separatorStyle = UITableViewCellSelectionStyleNone;
//    self.tableFirst.backgroundColor = [UIColor colorWithRed:242/255.0 green:242/255.0 blue:242/255.0 alpha:1.0];
    self.tableFirst.scrollEnabled = NO;
    self.tableFirst.delegate = self;
    self.tableFirst.dataSource = self;
    [self.tableFirst registerNib:[UINib nibWithNibName:@"WJDcellTableViewCell" bundle:nil] forCellReuseIdentifier:@"cellFirst"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == self.tableFirst) {
        NSLog(@"self.dataSourceFirst.count = %lu",(unsigned long)self.dataSourceFirst.count);
...
- (void)changeCellTextStatueColorWith:(UITableViewCell *)cell{
    cell.textLabel.textColor = self.unSelectedColor ? self.unSelectedColor : [UIColor whiteColor];
    cell.textLabel.highlightedTextColor = self.selectedColor ? self.selectedColor : [UIColor whiteColor];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.tableFirst) {
        ...
        WJDcellTableViewCell *cell1 = [self.tableFirst dequeueReusableCellWithIdentifier:@"cellFirst" forIndexPath:indexPath];
//        UIImageView *oneImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 8, 20, 20)];
//        UILabel *onelabel  =[[ UILabel alloc]initWithFrame:CGRectMake(25, 8, 40, 20)];
//        if (cell1 == nil) {
//            cell1 = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellID];
//      
//            [cell1.contentView addSubview:oneImage];
//            [cell1.contentView addSubview:onelabel];
//        }
        if (self.dataImageFirst.count != 0&&[self.dataImageFirst[0]isEqualToString:@"点诺"]) {
           cell1.oneImage.image = [UIImage imageNamed:self.dataImageFirst[indexPath.row]];
        }else if(self.dataImageFirst.count == 0){
    
        }else{
            NSString *strr = [NSString stringWithFormat:urlPicture,self.dataImageFirst[indexPath.row]];
            [cell1.oneImage sd_setImageWithURL:[NSURL URLWithString:strr] placeholderImage:nil];
        }
        cell1.onelabel.text = self.dataSourceFirst[indexPath.row];
        cell1.onelabel.font = self.tableTitleFont ? [UIFont systemFontOfSize:self.tableTitleFont] : TableTitleDefalutFont;
        cell1.backgroundColor = [UIColor colorWithRed:242/255.0 green:242/255.0 blue:242/255.0 alpha:1.0];
//        if (cell1.highlighted) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    void (^complete)(void) = ^(void){
        ...
        if (tableView == weakSelf.tableFirst) {
           // [btn setTitle:weakSelf.dataSourceFirst[indexPath.row] forState:UIControlStateNormal];
//            if (_delegate&&[_delegate respondsToSelector:@selector(menuCellDiDSelected:firstIndex:)]) {
//                [_delegate menuCellDiDSelected:weakSelf.lastSelectedIndex - tempTag firstIndex:indexPath.row];
//            }
    ...
    if (tableView == self.tableFirst) {
        ...
        if (self.isNet&&[self.dataSourceFirst[0]isEqualToString:@"外科"]) {
            NSLog(@"lalalallaalalallalalal");
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(netMenuClickMenuIndex:menuTitle:FirstIndex:firstContent:)]) {
                ...
                NSLog(@"sdjisjdiajidjaisjidjasidjiajdi");
            }
            if (_delegate&&[_delegate respondsToSelector:@selector(menuCellDiDSelected:firstIndex:)]) {
                [_delegate menuCellDiDSelected:self.lastSelectedIndex - tempTag firstIndex:indexPath.row];
                [self.tableSecond reloadData];
                
                NSLog(@"哒哒哒哒哒哒哒哒哒哒哒哒哒哒");
...
- (NSMutableArray *)allImageSource{
    if (!_allImageSource) {
        self.allImageSource = [NSMutableArray array];
    }return _allImageSource;
}

- (NSMutableArray *)dataImageFirst{
    if (!_dataImageFirst) {
        self.dataImageFirst = [NSMutableArray array];
    }return _dataImageFirst;
}
...
- (void)netLoadFirstArray:(NSArray *)firstArray FirstImageArray:(NSArray *)firstImageArray{
    [self createWithFirstData:firstArray firstImage:firstImageArray];
