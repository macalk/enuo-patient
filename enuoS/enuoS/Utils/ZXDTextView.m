...
#pragma mark - ZXDIndexPath


@implementation ZXDIndexPath
...
+ (instancetype)indexPathWithColumn:(NSInteger)column row:(NSInteger)row {
    ZXDIndexPath *path = [[self alloc] initWithColumn:column row:row];
    return path;
}
...
@end
#pragma mark - ZXDTextView



@interface ZXDTextView ()<UITableViewDataSource,UITableViewDelegate>
{
    struct {
...
//数据处理
- (void)setDataSource:(id<ZXDTextViewDataSource>)dataSource{
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:0 inColumn:i] > 0 && _dataSourceFlag.numberOfItemsInRow) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:i row:0 item:0]];
        }else {
            titleString = [_dataSource menu:self titleForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:i row:0]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            if (_dataSourceFlag.imageNameForRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageNameForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    // cell.imageView.image = [UIImage imageNamed:imgName];
                    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imgName]];
                }else {
            ...
            if (_dataSourceFlag.detailTextForRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            }else {
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            ...
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            if (_dataSourceFlag.imageNameForItemInRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    // cell.imageView.image = [UIImage imageNamed:imgName];
                    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imgName]];
                }else {
            ...
            if (_dataSourceFlag.detailTextForItemInRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            }else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _leftTableView) {
        ...
        if (isClickHaveItemValid && _delegate &&[_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:[_currentSelectedRows[_currentSelectedColumn] integerValue] item:indexPath.item]];
        }
    }
}
...
- (void)selectDeafultIndexPath {
    [self selectIndexPath:[ZXDIndexPath indexPathWithColumn:0 row:0]];
}
//获取IndexPath所对应的字符串
...
- (void)selectIndexPath:(ZXDIndexPath *)indexPath {
    ...
    if (indexPath.item < 0 ) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
            [_delegate menu:self didSelectRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:indexPath.column row:indexPath.row item:0]];
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
        ...
        //[self.currentSelectedRows removeAllObjects];
...
- (BOOL)setMenuWithSelectedRow:(NSInteger)row {
    ...
    if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn] > 0) {
        if (_isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:row]];
...
- (void)setMenuWithSelectedItem:(NSInteger)item {
    ...
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZXDIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedMenudRow item:item]];
    [self animateIndicator:_indicators[_currentSelectedColumn] background:_backGroundView tableView:_leftTableView title:title reverse:NO complecte:^{
        _isShow = NO;
    }];
}
