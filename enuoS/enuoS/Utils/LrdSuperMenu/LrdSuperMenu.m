...
@property (nonatomic,assign)NSInteger addiSum;
@property (nonatomic,assign)NSInteger addjSum;
@end
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //初始化 筛选tableView
        _onlyTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y + self.frame.size.height, SCREEN_WIDTH / 2, 0) style:UITableViewStylePlain];
        _onlyTableView.delegate = self;
        _onlyTableView.dataSource = self;
        _onlyTableView.backgroundColor = [UIColor colorWithRed:242/255.0 green:242/255.0 blue:242/255.0 alpha:1.0];
        _onlyTableView.rowHeight = kTableViewCellHeight;
        _onlyTableView.separatorColor = kSeparatorColor;
...
//    NSInteger touchIndex = touchPoint.x / (SCREEN_WIDTH / self.numberOfColumn);
    ...
    if (show) {
        if (haveItems) {
            ...
            //_onlyTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, 0);
           ...
           // [self.superview addSubview:_onlyTableView];
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_rightTableView];
        }else {
            if (self.markSum ==2) {
                _onlyTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                _onlyTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, 150);                _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, tableViewHeight);
                ...
                _onlyTableView.frame  =  CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, 150);
            }
        }];
    }else {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            }else if (_onlyTableView){
                [_onlyTableView removeFromSuperview];
            }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    }else if(tableView == _onlyTableView){
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (self.markSum == 2 &&_dataSourceFlag.titleForRowsAtIndexPath) {
           UIView *aView = [[UIView alloc]init];
        
        
        UILabel *alebel = [[UILabel alloc]init];
        alebel.font = [UIFont systemFontOfSize:14];
        alebel.text = @"实施方法";
        alebel.textColor = [UIColor lightGrayColor];
        alebel.frame = CGRectMake(10, 5, SCREEN_WIDTH, 20);
       
        [aView addSubview:alebel];
        UIButton *oneButton = [UIButton buttonWithType:UIButtonTypeCustom];
        UIButton *twoButton = [UIButton buttonWithType:UIButtonTypeCustom];
        
        [oneButton setTitle:@"重置" forState:UIControlStateNormal];
        [twoButton setTitle:@"确定" forState:UIControlStateNormal];
        [oneButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        [twoButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        ...
        twoButton.backgroundColor = [UIColor colorWithRed:255/255.0 green:192/255.0 blue:202/255.0 alpha:1.0];
        ...
        if ([_dataSource menu:self numberOfRowsInColumn:_currentSelectedColumn]<3) {
            self.addjSum = [_dataSource menu:self numberOfRowsInColumn:_currentSelectedColumn];
            self.addiSum = 1;
          
            for (int i = 0; i< self.addjSum ; i++) {
               
                UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
                [button setTitle:[_dataSource menu:self titleForRowAtIndexPath:[LrdIndexPath indexPathWithColumn:_currentSelectedColumn row:i]] forState:UIControlStateNormal];
                button.frame = CGRectMake(i *((SCREEN_WIDTH -40)/3+10) +10, 30, (SCREEN_WIDTH -40)/3, 30);
                button.tag = 100 +i;
                button.titleLabel.font = [UIFont systemFontOfSize:14];
               
                [button setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
                [aView addSubview:button];
                 [button setBackgroundImage:[UIImage imageNamed:@"组-3-拷贝-9"] forState:UIControlStateNormal];
                [button addTarget:self action:@selector(handleWithTwoMenubtn:) forControlEvents:UIControlEventTouchUpInside];
                
                
            }
            aView .frame = CGRectMake(0, 0, SCREEN_WIDTH, 50);
            return aView;
            
        }else{
            self.addjSum = [_dataSource menu:self numberOfRowsInColumn:_currentSelectedColumn]/3 +1;
            self.addiSum = [_dataSource menu:self numberOfRowsInColumn:_currentSelectedColumn] - (self.addjSum - 1)*3;
           NSInteger n = 0;
            for (int i = 0; i  < self.addjSum;i++) {
                for (int j = 0; j<3; j++) {
                    n ++;
                    if (n == [_dataSource menu:self numberOfRowsInColumn:_currentSelectedColumn]) {
                        break;
                    }
                    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
                    [button setTitle:[_dataSource menu:self titleForRowAtIndexPath:[LrdIndexPath indexPathWithColumn:_currentSelectedColumn row:n]] forState:UIControlStateNormal];
                    button.frame = CGRectMake(j *((SCREEN_WIDTH -40)/3+10) +10, 30 +(30 +10)*i, (SCREEN_WIDTH -40)/3, 30);
                    button.tag = 200 +n;
                       button.titleLabel.font = [UIFont systemFontOfSize:14];
                  
                     [button setBackgroundImage:[UIImage imageNamed:@"组-3-拷贝-9"] forState:UIControlStateNormal];
                    //button.titleEdgeInsets = UIEdgeInsetsMake(0,(SCREEN_WIDTH -40)/3,0,0);
            ...
            aView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 20 +30*self.addjSum);
            return aView;
...
//3menu上的 buttOn;
- (void)handleWithTwoMenubtn:(UIButton *)sender{
    if(sender.isSelected == NO){
        //记得昨晚操作之后，改变按钮的点击状态
        //[sender setImage:[UIImage imageNamed:@"789"] forState:UIControlStateNormal];
        //self.registerButton.backgroundColor = [UIColor colorWithRed:0/255.0 green:179/255.0 blue:163/255.0 alpha:1];
        [sender setBackgroundImage:[UIImage imageNamed:@"789"] forState:UIControlStateNormal];
        sender.selected = YES;
    }else{
        
        [sender setBackgroundImage:[UIImage imageNamed:@"组-3-拷贝-9"] forState:UIControlStateNormal];
...
//        }
    ...
    if (tableView == _leftTableView ) {
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            ...
            UIImageView *markImage = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@""] highlightedImage:[UIImage imageNamed:@"蓝色线"]];
...
//        }
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            UIImageView *markImage = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"圆框-拷贝-2"] highlightedImage:[UIImage imageNamed: @"点击选择"]];
           ...
           // __weak  typeof(self) weakSelf = self;
            [markImage mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.centerX.mas_equalTo(cell.contentView).with.offset(50);
                make.centerY.equalTo(cell.contentView);
            }];
...
- (void)menuTapped:(UITapGestureRecognizer *)gesture {
    ...
    CGPoint touchPoint = [gesture locationInView:self];
    NSInteger touchIndex = touchPoint.x / (SCREEN_WIDTH / self.numberOfColumn);
       NSLog(@"啦啦啦啦啦啦%ld",touchIndex);
    
    self.markSum = touchIndex;
    ...
    if (touchIndex == _currentSelectedColumn && _isShow) {
        //收回menu
        if (touchIndex == 2) {
            [self animateIndicator:_indicators[touchIndex] background:_backGroundView tableView:_onlyTableView title:_titles[touchIndex] reverse:NO complecte:^{
...
//    }else if(touchIndex == 2 &&touchIndex == _currentSelectedColumn && _isShow ) {
//        
//        [self animateIndicator:_indicators[touchIndex] background:_backGroundView tableView:_onlyTableView title:_titles[touchIndex] reverse:NO complecte:^{
//            _isShow = NO;
//        }];
//
//    }else if (touchIndex == 2&&touchIndex == _currentSelectedColumn && !_isShow){
//        [self animateIndicator:_indicators[touchIndex] background:_backGroundView tableView:_onlyTableView title:_titles[touchIndex] reverse:YES complecte:^{
//            _isShow = YES;
//        }];
    }else{
        //弹出menu
